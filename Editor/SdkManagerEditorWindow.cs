...
namespace xClouder.SdkManager{
	public class SdkManagerEditorWindow : EditorWindow {
		...
		void OnGUI()
		{
			...
			{
				GUILayout.Label("There is no SDKs installed, \nyou can install sdks by click 'Install' button. \nFor more information, click the 'Help' button. \nEnjoy!");
			}
			...
			{
				Application.OpenURL("https://github.com/xclouder/SdkManager/tree/master");
			}
			...
			if (GUILayout.Button("Install", new GUILayoutOption[] { GUILayout.Width(80)}))
			{
				...
				if (!string.IsNullOrEmpty(path) && System.IO.Directory.Exists(path))
				{
					...
					{
						Debug.LogWarning("Install failed, try to uninstall. Error:" + ioe.ToString());
						try
						{
						...
						{
							EditorUtility.DisplayDialog("Uninstall failed", ioe2.ToString(), "ok");
						}
					}
